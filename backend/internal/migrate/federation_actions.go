...
func RunFederationActions(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if err != nil {
		return fmt.Errorf("migrate federation actions: check posts: %w", err)
	}
	hasFederationIncoming, err := tableExists(ctx, pool, "federation_incoming_posts")
	if err != nil {
		return fmt.Errorf("migrate federation actions: check federation incoming: %w", err)
	}
	steps := []string{}
	if hasPosts {
		steps = append(steps,
	...
	hasOutboxDeliveries, err := tableExists(ctx, pool, "glipz_protocol_outbox_deliveries")
	if err != nil {
		return fmt.Errorf("migrate federation actions: check outbox deliveries: %w", err)
	}
	if hasOutboxDeliveries {
	...
	hasUnlocks, err := tableExists(ctx, pool, "federation_incoming_post_unlocks")
	if err != nil {
		return fmt.Errorf("migrate federation actions: check federation incoming unlocks: %w", err)
	}
	if !hasUnlocks {
		return nil
	}
