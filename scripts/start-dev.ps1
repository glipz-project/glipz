...
FRONTEND_ORIGIN=http://127.0.0.1:8080,http://127.0.0.1:5173,http://localhost:5173
GLIPZ_MEDIA_PROXY_MODE=proxy
GLIPZ_AUTH_RATE_LIMIT_FAIL_CLOSED=true
GLIPZ_FEDERATION_KEY_SEED=$seed
PATREON_ENABLED=false
"@ | Set-Content -LiteralPath '.env.dev' -Encoding ascii
}
New-Item -ItemType Directory -Force -Path 'data/media', 'data/legal-docs' | Out-Null
docker compose --env-file .env.dev -f docker-compose.dev.yml up -d --build --wait --wait-timeout 180
