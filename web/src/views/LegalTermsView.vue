...
const { tm } = useI18n();
const overviewPoints = computed(() => tm("legalTerms.overviewPoints") as string[]);
const prohibitedItems = computed(() => tm("legalTerms.prohibitedItems") as string[]);
const sections = computed(() => tm("legalTerms.sections") as Array<{ title: string; paragraphs: string[] }>);
const accountParagraphs = computed(() => tm("legalTerms.accountParagraphs") as string[]);
const supplementParagraphs = computed(() => tm("legalTerms.supplementParagraphs") as string[]);
const { customDocHtml, customDocUpdatedDate } = useLegalMarkdownDoc("terms");
    ...
    <div class="mx-auto flex w-full max-w-6xl flex-col gap-8">
      ...
      <section class="overflow-hidden rounded-[2rem] border border-lime-200 bg-white">
        <div class="grid gap-8 px-6 py-10 sm:px-8 lg:grid-cols-[minmax(0,1.1fr)_24rem] lg:items-center lg:px-10">
          <div class="max-w-3xl">
            <p class="text-sm font-semibold uppercase tracking-[0.2em] text-lime-700">{{ $t("legalTerms.badge") }}</p>
            <h1 class="mt-4 text-4xl font-bold tracking-tight text-neutral-900 sm:text-5xl">
              {{ ($tm("legalTerms.title") as string[])[0] }}
              <br />
              {{ ($tm("legalTerms.title") as string[])[1] }}
            </h1>
          ...
          <div class="rounded-xl border border-neutral-200 bg-white/90 p-5 shadow-sm">
            <p class="text-sm font-semibold text-neutral-900">{{ $t("legalTerms.overviewTitle") }}</p>
      ...
      <section class="rounded-[2rem] border border-neutral-200 bg-white px-6 py-8 sm:px-8">
        ...
        <div v-else class="space-y-8">
          <section>
            <h2 class="text-xl font-semibold text-neutral-900">{{ $t("legalTerms.prohibitedTitle") }}</h2>
            <p class="mt-3 text-sm leading-7 text-neutral-700">{{ $t("legalTerms.prohibitedIntro") }}</p>
            <ul class="mt-4 space-y-3 text-sm leading-7 text-neutral-700">
              <li v-for="item in prohibitedItems" :key="item" class="flex gap-3">
                <span class="mt-2 h-2 w-2 shrink-0 rounded-full bg-lime-500" />
                <span>{{ item }}</span>
              </li>
            </ul>
          ...
          <section class="border-t border-neutral-200 pt-8">
            <h2 class="text-xl font-semibold text-neutral-900">{{ $t("legalTerms.accountTitle") }}</h2>
            <p v-for="paragraph in accountParagraphs" :key="paragraph" class="mt-3 text-sm leading-7 text-neutral-700">
              {{ paragraph }}
            </p>
          </section>
          ...
          <section class="border-t border-neutral-200 pt-8">
            <h2 class="text-2xl font-semibold text-neutral-900">{{ $t("legalTerms.supplementTitle") }}</h2>
