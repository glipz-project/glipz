...
async function loadItems() {
  ...
  items.value = await listMyCustomEmojis(token);
...
async function createEmoji() {
  ...
  notice.value = "";
  try {
    const item = await createMyCustomEmoji(token, {
    ...
    notice.value = t("views.customEmojiSettings.created");
  } catch (e: unknown) {
    error.value = e instanceof Error ? e.message : t("views.customEmojiSettings.errors.create");
  } finally {
...
async function saveEmoji(item: CustomEmoji) {
  ...
  notice.value = "";
  try {
    const updated = await patchMyCustomEmoji(token, item.id, draft);
    items.value = items.value.map((row) => (row.id === item.id ? updated : row));
    syncDrafts(items.value);
    await refreshCustomEmojiCatalog(token);
    notice.value = t("views.customEmojiSettings.saved");
  } catch (e: unknown) {
    error.value = e instanceof Error ? e.message : t("views.customEmojiSettings.errors.save");
  } finally {
    rowBusy.value = null;
  }
}
...
async function removeEmoji(item: CustomEmoji) {
  ...
  notice.value = "";
  try {
    await deleteMyCustomEmoji(token, item.id);
    items.value = items.value.filter((row) => row.id !== item.id);
    delete drafts[item.id];
    await refreshCustomEmojiCatalog(token);
    notice.value = t("views.customEmojiSettings.deleted");
  } catch (e: unknown) {
    error.value = e instanceof Error ? e.message : t("views.customEmojiSettings.errors.delete");
  } finally {
    rowBusy.value = null;
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : t("views.customEmojiSettings.errors.load");
  } finally {
    loading.value = false;
  }
});
      ...
      <section class="mt-6 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2">
          <label class="block text-sm">
            <span class="mb-1 block font-medium text-neutral-800">{{ $t("views.customEmojiSettings.shortcodeLabel") }}</span>
            <input
              ...
              :placeholder="$t('views.customEmojiSettings.shortcodePlaceholder')"
            />
          </label>
          <label class="block text-sm">
        ...
        <label class="mt-4 flex items-center gap-2 text-sm text-neutral-700">
          ...
          <span>{{ $t("views.customEmojiSettings.enabledLabel") }}</span>
        </label>
        <div class="mt-4 flex items-center justify-between gap-3">
          <p class="text-sm text-neutral-500">{{ $t("views.customEmojiSettings.shortcodeHint") }}</p>
          <button
            type="button"
            ...
            @click="createEmoji"
          >
            {{ createBusy ? $t("views.customEmojiSettings.createBusy") : $t("views.customEmojiSettings.create") }}
          </button>
        </div>
      </section>
        ...
        <div v-else class="mt-4 space-y-4">
          <article v-for="item in items" :key="item.id" class="rounded-xl border border-neutral-200 bg-neutral-50 p-4">
            <div class="flex items-start justify-between gap-4">
              <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                  ...
                  <p class="mt-1 text-xs text-neutral-500">{{ item.is_enabled ? $t("views.customEmojiSettings.enabledState") : $t("views.customEmojiSettings.disabledState") }}</p>
                </div>
              </div>
              <button
                type="button"
              ...
              >
                {{ $t("views.customEmojiSettings.delete") }}
              </button>
            </div>
            ...
            <div class="mt-4 flex justify-end">
              ...
              >
                {{ rowBusy === item.id ? $t("views.customEmojiSettings.saveBusy") : $t("views.customEmojiSettings.save") }}
              </button>
            </div>
          </article>
        </div>
