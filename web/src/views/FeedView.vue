...
<template>
  ...
  <PullToRefresh :on-refresh="refreshFeed">
    ...
    <EmptyState v-if="err" :title="err" :action-label="$t('ux.retry')" @action="refreshFeed" />

    <EmptyState v-if="feedLoading && !items.length && !err" :title="$t('views.feed.loading')" busy />
    ...
    <PostTimeline class="ui-timeline"
      :items="items"
